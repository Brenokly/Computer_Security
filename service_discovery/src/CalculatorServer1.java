...
import java.net.ServerSocket; // Importa ServerSocket
...
public class CalculatorServer1 {
  ...
  private static final int MY_PORT = 9090;
  ...
  public static void main(String[] args) {
    ...
    try (ServerSocket serverSocket = new ServerSocket(MY_PORT)) {
      System.out.println("==================================================");
      System.out.println(" Calculadora 1 (" + MY_ADDRESS + ") escutando por cálculos...");
      System.out.println("==================================================");

      while (true) {
        Socket clientSocket = serverSocket.accept();
        System.out.println("[CalcServer1] Novo cliente de cálculo conectado: " + clientSocket.getInetAddress());
        new CalculationHandler(clientSocket).start();
      }

    } catch (Exception e) {
      System.err.println("Erro fatal na Calculadora 1 (Servidor): " + e.getMessage());
  ...
  private static void registerServices() {
    ...
    try (
        Socket socket = new Socket(DIRECTORY_ADDRESS, DIRECTORY_PORT);
        PrintWriter writer = new PrintWriter(socket.getOutputStream(), true, StandardCharsets.UTF_8);
        BufferedReader reader = new BufferedReader(
            new InputStreamReader(socket.getInputStream(), StandardCharsets.UTF_8))) {

      System.out.println("[Registro Calc1] Conectado ao Diretório. Registrando...");
      ...
      for (String service : services) {
        ...
        if (!"REGISTER_OK".equals(response)) {
          System.err.println("[Registro Calc1] Falha ao registrar " + service);
        }
      }
      System.out.println("[Registro Calc1] Todos os serviços registrados.");

    } catch (Exception e) {
      System.err.println("Erro no registro da Calculadora 1: " + e.getMessage());
    }
    // A thread de registro morre aqui, mas o servidor (main) continua rodando.
  }
